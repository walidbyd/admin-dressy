        ...
        class="text-white mb-5 sm:mb-6 lg:mb-8"
         iconColor="white">{{status.msg}}</ps-banner-icon>
            ...
            <ps-table2 :row="row" :search="search" :object="menu_groups" :colFilterOptions="colFilterOptions"
                ...
                <template #button>
                   <ps-button v-if="can.createVendorMenuGroup" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" />{{$t('core_add_vendor_menu_group_module')}}</ps-button>
                </template>
