<template>
    <ps-layout :can="can">
        ...
        <ps-table2 :row="row" :search="search" :object="this.roles" :colFilterOptions="colFilterOptions"
            ...
            <template #button>
                    <ps-button v-if="can.createVendorRole" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" /> {{ $t('core__be_add_vendor_role') }}</ps-button>
            </template>
