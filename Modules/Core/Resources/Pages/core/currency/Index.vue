<template>
    ...
    <ps-layout>
        ...
        @handleSort="handleSorting"  @handleSearch="handleSearching" @handleRow="handleRow"
            ...
            <!-- add new field start -->
            <template #button>
                <ps-button v-if="can.createItemCurrency" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-2 font-semibold" />{{$t('core__add_currency')}}</ps-button>
            </template>
