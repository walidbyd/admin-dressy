<template>
    ...
    <ps-layout>
        ...
        <ps-table2 :customizeDetails="customizeDetails" :row="row" :search="search" :object="this.users" :colFilterOptions="colFilterOptions"
            ...
            <template #button>
                    <ps-button v-if="can.createUser" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" /> {{ $t('core__be_add_user') }}</ps-button>
            </template>
