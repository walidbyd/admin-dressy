    ...
    <ps-layout>
        ...
        <ps-table2 :row="row" :search="search" :object="backend_languages" :colFilterOptions="colFilterOptions"
            ...
            <template #tableRow="rowProps">
                ...
                <div v-if="rowProps.field == 'is_publish'">
                    <ps-toggle :disabled="!rowProps.row.authorizations.update" :selectedValue="rowProps.row.is_publish == '1' ? true : false" @click="handleIsPublish(rowProps.row.id)"></ps-toggle>
                </div>
            </template>
        </ps-table2>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        return { columns, ps_danger_dialog, ps_loading_circle_dialog, confirmDeleteClicked, colFilterOptions,handleSorting ,handleSearching ,handleRow ,visible,languageStore, handleIsPublish}
    },
    computed: {
        breadcrumb() {
            return [
    ...
    methods: {
        ...
        handlePublish(row){
            this.$inertia.put(route('language.statusChange',row.id), '', {
                onSuccess: () => {
                    if(this.$page.props.status.flag == 'success') {
                        loadLanguageAsync(row.symbol);
                        this.languageStore.setActiveLanguage(row.symbol);
