                                ...
                                <div v-if="title == settingColumn[0].label">
                                    ...
                                    <div class="px-4 py-3">
                                        ...
                                        <ps-input type="text" v-model:value="form.delivery_charges" :placeholder="$t('vendor_panel__delivery_charges')"
                                            @keypress="checkDeliveryCharges($event)"
                                            @paste="onPaste"
...
<script>
import { defineComponent, ref, defineAsyncComponent, reactive, onMounted, onUnmounted, nextTick } from 'vue';
...
import PsCheckboxValue from "@/Components/Core/Checkbox/PsCheckboxValue.vue";
    ...
    export default defineComponent({
        ...
        setup(props){
            ...
            function checkDeliveryCharges(e) {
                const char = e.key;
            ...
            const onPaste = (e) => {
                ...
                const pastedText = (e.clipboardData || window.clipboardData).getData("text"); // Get pasted text
                const sanitizedText = form.delivery_charges + pastedText.replace(/[^0-9]/g, ""); // Remove non-numeric characters

                // Insert only numbers into the input
                nextTick(() => {
                    form.delivery_charges = sanitizedText;
                });
            };
